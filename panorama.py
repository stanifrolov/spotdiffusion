    ...
    def text2panorama(self, prompts, negative_prompts='', height=512, width=2048, num_inference_steps=50,
        ...
        with torch.autocast('cuda'):
            for i, t in enumerate(tqdm(self.scheduler.timesteps)):
                ...
                # apply the SpotDiffusion shift, efficiently implemented via horizontal duplication and random crop
                latent = torch.cat((value, value), -1)
                crop_idx = torch.randint(0, width // 8, (1,)).item()
                latent = latent[:, :, :, crop_idx:width // 8 + crop_idx]
...
if __name__ == '__main__':
    ...
    sd = SpotDiffusion(device, opt.sd_version)
